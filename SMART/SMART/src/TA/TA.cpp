...
TransmissionAgent::TransmissionAgent(std::string routerip){
...
int TransmissionAgent::encapsulate(nfq_q_handle *myQueue, nfgenmsg *msg, nfq_data *pkt, void *cbData) {
    ...
    if (sendto(ta->socketForwarder, message, msgLenght, 0, (struct sockaddr*) &forwarderAddr, sizeof(forwarderAddr)) == -1)
        perror("error sendto()");
