...
void RoutingManagerBase::run() {
    ...
    while (true)   {
        ...
        pathManager->doMonitorPaths(LATENCY_METRIC, pathsToTest);

        // wait for TIMEOUT
        std::this_thread::sleep_for(std::chrono::milliseconds(PROBE_TIMEOUT_IN_MS));

        updateRoutingTable();

        std::cout << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(PROBE_PERIOD_IN_S));
    }
}

void RoutingManagerBase::runSimulation(){
	for (IPv4Address dest : index_destinations) {
		IPPath internetPath;
		internetPath.push_back(dest);
		currentRoutes[dest] = internetPath;
	...
	while (simulationManager->endMatrixResult == false)   {
		if (simulationManager->readResultsFromFile() != 0) {
			for (size_t i = 0; i < index_origin.size(); i++) {
				...
				//simulationManager->run(LATENCY_METRIC, pathsToTest);
				if (simulationManager->run(BW_METRIC, pathsToTest) != 0) {
					...
					simulationManager->writeResultInFile(proposedPaths);
				}

			}
		} else {
			std::cout << "endMatrixResult" << std::endl;
